...
Options::Options(float screenWidth, float screenHeight)
{
	...
	options[0].setColor(sf::Color::Green);
	...
	options[4].setPosition(sf::Vector2f(screenWidth / 9, screenHeight / (MAX_NUMBER_OF_ITEMS + 1) * 5));
	options[4].setCharacterSize(50);

        options[5].setFont(font);
	options[5].setColor(sf::Color::White);
	OptionsText = "Attack Key - " + s.keyToStr(s.getAttackKey());
	options[5].setString(OptionsText);
	options[5].setPosition(sf::Vector2f(screenWidth / 9, screenHeight / (MAX_NUMBER_OF_ITEMS + 1) * 6));
	options[5].setCharacterSize(50);

	options[6].setFont(font);
	options[6].setColor(sf::Color::White);
	OptionsText = "Quit Key - " + s.keyToStr(s.getQuitKey());
	options[6].setString(OptionsText);
	options[6].setPosition(sf::Vector2f(screenWidth / 9, screenHeight / (MAX_NUMBER_OF_ITEMS + 1) * 7));
	options[6].setCharacterSize(50);

        options[7].setFont(font);
	options[7].setColor(sf::Color::White);
	OptionsText = "Restore Settings to Defaults";
	options[7].setString(OptionsText);
	options[7].setPosition(sf::Vector2f(screenWidth / 9, screenHeight / (MAX_NUMBER_OF_ITEMS + 1) * 8));
	options[7].setCharacterSize(50);

        options[8].setFont(font);
	options[8].setColor(sf::Color::White);
	OptionsText = "Return to Menu";
	options[8].setString(OptionsText);
	options[8].setPosition(sf::Vector2f(screenWidth / 9, screenHeight / (MAX_NUMBER_OF_ITEMS + 1) * 9));
